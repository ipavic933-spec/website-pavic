...
- Language Provider - Client context that exposes fixed Croatian (`hr`) translations and a no-op language setter for API compatibility.
- Language Switcher - Header control with two visual buttons (`HR`, `EN`) that keep `aria-pressed`/`aria-disabled` states but do not change displayed language.
...
- Displayed language remains fixed to Croatian (`hr`) for all sections.
